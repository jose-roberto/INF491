    ...
    def __init__(
        ...
        for i, part in enumerate(self._rep.parts):
            ...
            for el in part.flatten().notes:
                ...
                inicio = float(el.offset) * self._spb
                ...
                dur = float(el.duration.quarterLength) * self._spb
                ...
                if isinstance(el, m21.chord.Chord):
                    for nota in el.notes:
                        f = midi2freq(nota.pitch.midi)                        
                        parte['eventos'].append((f, inicio, dur, amp))
            ...
            self.parts[-1]['eventos'].sort(key=lambda evento: evento[1])
...
class Player:
    ...
    def setInstrumentos(
        ...
        for item in instrumentos:
            if isinstance(item, tuple) and len(item) == 2:
                ...
                inst = item
                ganho = 1.0  # Ganho padrão neutro 

            if not isinstance(inst, Instrumento):
                raise ValueError(f"Instrumento inválido: objeto de classe {type(inst)}.")

            if not isinstance(ganho, (int, float)):
                raise ValueError(f"Ganho inválido: {ganho}. Deve ser um valor numérico.")

            self.instrumentos.append(inst)
            self.ganhos.append(float(ganho))
        ...
        num_vozes = len(self.arqMIDI.parts)
        
        tempo_extra = 2.0 
        duracao_total = self.arqMIDI.duracao + tempo_extra
        
        # Converte a duração de segundos para número de amostras
        num_amostras = int(duracao_total * self.sr)
        
        self.buffers = np.zeros((num_vozes, num_amostras))
    ...
    def processa(
        ...
        for i in range(nparts):
            ...
            for evento in eventos:
                ...
                y = instrumento.gerar_nota(f=f, dur=dur, sr=self.sr, amp=amp)

                # Converte o início de segundos para índice de amostra
                ini = int(inicio * self.sr)
                
                fim = ini + len(y)

                # Ignora nota que começa depois que o buffer acabou
                if ini >= N:
                    continue
                
                # Corta nota que ultrapassa final do buffer
                if fim > N:
                    y = y[:(N - ini)]
                    fim = N

                self.buffers[i][ini:fim] += y
        ...
        for i in range(nparts):
            # Pico de amplitude máxima da voz
            pico_maximo = np.max(np.abs(self.buffers[i]))
            
            # Normaliza se o pico for maior que 1
            if pico_maximo > 1.0:
                self.buffers[i] = self.buffers[i] / pico_maximo
    ...
    def getWav(
        ...
        for i in range(len(self.ganhos)):
            buffers_mix[i] = buffers_mix[i] * self.ganhos[i]
        ...
        pico_maximo = np.max(np.abs(buffer))
        
        if pico_maximo > 1.0:
            buffer = buffer / pico_maximo
